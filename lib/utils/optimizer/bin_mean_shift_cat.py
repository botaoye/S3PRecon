...
class Bin_Mean_Shift_Cat(nn.Module):
    def __init__(self, train_iter=5, test_iter=10, bandwidth=0.5, device='cpu'):
        super(Bin_Mean_Shift_Cat, self).__init__()
    ...
    def generate_seed(self, point, bin_num):
        ...
        min_x, min_y, min_z, min_a, min_b = point.min(dim=0)[0]
        max_x, max_y, max_z, max_a, max_b = point.max(dim=0)[0]
        ...
        x = torch.linspace(start_x, end_x, bin_num).view(bin_num, 1, 1, 1, 1)
        y = torch.linspace(start_y, end_y, bin_num).view(1, bin_num, 1, 1, 1)
        z = torch.linspace(start_z, end_z, bin_num).view(1, 1, bin_num, 1, 1)
        a = torch.linspace(start_a, end_a, bin_num).view(1, 1, 1, bin_num, 1)
        b = torch.linspace(start_b, end_b, bin_num).view(1, 1, 1, 1, bin_num)

        x_repeat = x.repeat(1, bin_num, bin_num, bin_num, bin_num).view(-1, 1)
        y_repeat = y.repeat(bin_num, 1, bin_num, bin_num, bin_num).view(-1, 1)
        z_repeat = z.repeat(bin_num, bin_num, 1, bin_num, bin_num).view(-1, 1)
        a_repeat = a.repeat(bin_num, bin_num, bin_num, 1, bin_num).view(-1, 1)
        b_repeat = b.repeat(bin_num, bin_num, bin_num, bin_num, 1).view(-1, 1)

        return torch.cat((x_repeat, y_repeat, z_repeat, a_repeat, b_repeat), dim=1).to(self.device)
    ...
    def cal_distance_matrix(self, point_a, point_b):
        ...
        a_repeat = point_a.repeat(1, n).view(n * m, 5)                  # (n*m, 2)
