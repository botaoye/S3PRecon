...
def get_K_inv_dot_xy_1(h=480, w=640):
    focal_length_x = 577.590698
    focal_length_y = 578.729797
    offset_x = 318.905426
    offset_y = 242.683609
...
def writePLYFileDepth(folder, index, depth, segmentation, non_plane_index=20):
    # h, w = 192, 256
    h, w = 480, 640
    # imageFilename = str(index) + '_segmentation_pred_blended_0.png'
    imageFilename = str(index) + '.png'
    ...
    for y in range(h - 1):
        for x in range(w - 1):
            ...
            if segmentation[y + 1, x] == segmentIndex and segmentation[y + 1, x + 1] == segmentIndex and min(
                    depths) > 0 and max(depths) < 10:
                faces.append((x, y, x, y + 1, x + 1, y + 1))

            depths = [depth[y][x], depth[y][x + 1], depth[y + 1][x + 1]]
            if segmentation[y][x + 1] == segmentIndex and segmentation[y + 1][x + 1] == segmentIndex and min(
                    depths) > 0 and max(depths) < 10:
...
"""
        ...
        for y in range(h):
            for x in range(w):
                segmentIndex = segmentation[y][x]
                if segmentIndex == non_plane_index:
                    f.write("0.0 0.0 0.0\n")
                    continue
