...
def eval_render(network, data_loader, device, use_surface_render=False, save_dir='', cfg=None):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(tqdm(data_loader, desc='Rendering')):
            ...
            # scale_depth = (gt_depth / (rcnn_depth * valid_depth_mask + 0.0000000001)).mean()
            ...
            if not eval_plane_rcnn:
                ...
                # We apply mask_threshold = 0.85 when eval segmentation results
                # mask_threshold = 0.93  used for visualization
                mask_threshold = 0.85
            ...
            # cv2.imwrite(os.path.join(save_dir, 'mask_' + file_name.replace('.png', '.jpg')), mask)
...
def dissimilarity_map(rgb, aligned_norm):
    ...
    # We apply rgb = kornia.filters.median_blur(rgb, (9, 9)) when eval segmentation results
    # rgb = kornia.filters.median_blur(rgb, (5, 5))
    # rgb = kornia.filters.median_blur(rgb, (3, 3)) for visualization
...
def render_time_test(network, data_loader, device, use_surface_render=False, save_dir='', cfg=None):
    H, W = 480, 640
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(tqdm(data_loader, desc='Rendering')):
            ...
            if not eval_plane_rcnn:
                batch = to_cuda(batch, device)
                ret = network(batch, use_surface_render=False)
    ...
    infer_speed = (end_time - start_time) / total_iters
    print('infer_speed: ', infer_speed)
