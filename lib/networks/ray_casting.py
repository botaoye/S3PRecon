...
def root_finding_surface_points(
    ...
    with torch.no_grad():
        ...
        if isinstance(far, torch.Tensor):
            far = far[mask == 0]
        d_pred_out[
            mask == 0] = np.inf if fill_inf else far  # no intersections; or the first intersection is from outside to inside; or the 0-th point is occupied.
...
def sphere_tracing_surface_points(
    ...
    for _ in range(N_iters):
        ...
        surface_val = model.forward_surface(pts)
...
def surface_render(rays_o: torch.Tensor,
                   rays_d: torch.Tensor,
                   model,
                   near=0.0,
                   far=2.0,
                   calc_normal=True,
                   rayschunk=1000000, netchunk=1048576, batched=True, use_view_dirs=True, show_progress=False,
                   ray_casting_algo='',
                   ray_casting_cfgs={},
                   **not_used_kwargs):
    """
    ...
    with torch.no_grad():
        ...
        def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor):
            ...
            if ray_casting_algo == 'root_finding':
                ...
                d_pred_out, pt_pred, mask = sphere_tracing_surface_points(
                    model, rays_o, rays_d, batched=batched, near=near, far=far, N_iters=20)
                # d_pred_out, pt_pred, mask = sphere_tracing_surface_points(
                #     model.implicit_surface, rays_o, rays_d, batched=batched, **ray_casting_cfgs)
            else:
                raise NotImplementedError

            color, _, sdf, nablas = model.forward(pt_pred, view_dirs)
            color[~mask] = 0  # black
        ...
        if calc_normal:
            ...
            extras['normals_volume'] = normals

        # return colors, depths, extras
        return extras


def surface_render_(rays_o: torch.Tensor,
                   rays_d: torch.Tensor,
                   model,
                   near=0.0,
                   far=2.0,
    ...
    with torch.no_grad():
        ...
        def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor):
            ...
            color, plane_seg, plane_embedding, sdf, nablas = model.forward(pt_pred, view_dirs)
            color[~mask] = 0  # black
            # NOTE: all without grad. especially for nablas.
            return color.data, d_pred_out.data, nablas.data, mask.data, plane_embedding.data
        ...
        plane_embeddings = []
        for i in tqdm(range(0, rays_o.shape[DIM_BATCHIFY], rayschunk), disable=not show_progress):
            color_i, d_i, nablas_i, mask_i, plane_embedding_i = render_rayschunk(
                rays_o[:, i:i + rayschunk] if batched else rays_o[i:i + rayschunk],
                rays_d[:, i:i + rayschunk] if batched else rays_d[i:i + rayschunk]
            )
            colors.append(color_i)
            ...
            nablas.append(nablas_i)
            masks.append(mask_i)
            plane_embeddings.append(plane_embedding_i)
        colors = torch.cat(colors, DIM_BATCHIFY)
        depths = torch.cat(depths, DIM_BATCHIFY)
        nablas = torch.cat(nablas, DIM_BATCHIFY)
        masks = torch.cat(masks, DIM_BATCHIFY)
        plane_embeddings = torch.cat(plane_embeddings, DIM_BATCHIFY)

        extras = OrderedDict([
            ('implicit_nablas', nablas),
            ('mask_surface', masks),
            ('plane_embedding', plane_embeddings),
            ('rgb', colors),
            ('depths', depths / depth_ratio),
        ])
...
def surface_render__(rays_o: torch.Tensor,
                   rays_d: torch.Tensor,
                   model,
                   near=0.0,
                   far=2.0,
    ...
    with torch.no_grad():
        ...
        def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor):
            ...
            color, plane_embedding, sdf, nablas = model.forward(pt_pred, view_dirs)
            color[~mask] = 0  # black
            # NOTE: all without grad. especially for nablas.
            return color.data, d_pred_out.data, nablas.data, mask.data, plane_embedding.data
